...
var dtlsClient = require('node-mbed-dtls');
...
function Agent(opts, dtlsOpts, cb) {
  if (!(this instanceof Agent))
    return new Agent(opts, dtlsOpts)
  ...
  this._opts = opts
  this._init(dtlsOpts, cb)
}

util.inherits(Agent, events.EventEmitter)

Agent.prototype._init = function initSock(dtlsOpts, newCallback) {
  var that = this
  if (this._sock) {
    return
  }
  ...
  if (dtlsOpts) {
    ...
    var bigSocket = dtlsClient.connect(dtlsOpts, socket => {
      socket.on('data', msg => {
        console.log("Message: ", msg);
        var packet, message, outSocket
        ...
        outSocket = {
          ...
          family: that._opts.type === 'udp4' ? 'IPv4' : 'IPv6'
        };
        that._handle(msg, outSocket, socket)
      });
    });
    ...
    //console.log("These are our client opts:  ", JSON.stringify(this._opts, 4));
    this._sock = dgram.createSocket(this._opts.type, function(msg, rsinfo) {
      ...
      if (that._opts.port) {
        that._sock.bind(that._opts.port);
      };
    });
...
Agent.prototype.request = function request(url, dtlsOpts) {
  this._init(dtlsOpts);
  var req, response, options = url.options || url.headers,
    option, that = this,
